...
def ventana_gestion(viaje):
    ...
    with st.form("form_gestion"):
        ...
        with c1:
            ...
            mot_b = c1.selectbox("Motivo Bus:", ["Operación Normal", "Falta de movil", "Bus varado", "Accidente", "Vandalismo", "Mantenimiento"])

        with c2:
            st.write("👤 **Operador**")
            ope_r = c2.text_input("Operador Real", value=viaje['ope_prog'])
            mot_o = c2.selectbox("Motivo Operador:", ["Operación Normal", "Falta de operador", "Enfermo", "No llegó", "Sanción"])
...
if is_admin:
    with tabs[-1]:
        ...
        with st.expander("🚀 Sincronización Rigel"):
            c1, c2 = st.columns(2)
        ...
        with st.form("u_reg"):
            ...
            nom, cor = st.text_input("Nombre"), st.text_input("Correo")
            car, pas = st.selectbox("Cargo", ["Auxiliar", "Técnico", "Coordinador"]), st.text_input("Clave")
