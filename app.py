...
def ventana_gestion(s_data):
    ...
    with st.form("f_gestion"):
        ...
        if st.form_submit_button("✅ GUARDAR EN NUBE", use_container_width=True):
            ...
            processor.registrar_ejecucion_gsheet({"servbus": s_data['servbus'], "bus_real": bn, "ope_real": on, "soc": f"{soc}%", "estado": estado}, st.session_state.user_info["nombre"])
...
if df is not None and not df.empty:
    ...
    with tabs[0]: # DASHBOARD
        st.metric("Servicios del Día", len(df_f))
    ...
    for i in range(3):
        with tabs[i]: st.info("No hay datos. Sincronice la semana en la pestaña ADMIN.")
...
with tabs[3]:
    ...
    with st.expander("📅 Carga Semanal desde Rigel", expanded=True):
        ...
        if st.button("🚀 DESCARGAR SEMANA A GOOGLE SHEETS", use_container_width=True):
            if processor.sincronizar_rango_rigel(str(f_i), str(f_f)):
                st.success("¡Semana cargada!"); st.rerun()
            else: st.error("Error. Revise VPN/Ngrok.")
